...
public class BlackContentProvider extends ContentProvider {
    ...
    static {
        ...
        MATCHER.addURI("com.tricheer.blacklist", "blackdata", CODE_NOPARAM);

    }

    @Override
    public boolean onCreate() {
        dbHelper = new DatabaseHelper(this.getContext(), "blacklist.db", null, 1);
        return true;
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        if (db.isOpen()) {
            return db.query("blackdata", projection, selection, selectionArgs, null, null, sortOrder);
        }
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        if (db.isOpen()) {
            // 特别说一下第二个参数是当name字段为空时，将自动插入一个NULL。
            long rowid = db.insert("blackdata", "null", contentValues);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            int count = db.delete("blackdata", selection, selectionArgs);
            this.getContext().getContentResolver().notifyChange(uri, null);
            db.close();
            return count;
        }
    ...
    public int update(Uri uri, ContentValues contentValues, String selection, String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            int count = db.update("blackdata", contentValues, selection, selectionArgs);
            this.getContext().getContentResolver().notifyChange(uri, null);
            db.close();
            return count;
        }
