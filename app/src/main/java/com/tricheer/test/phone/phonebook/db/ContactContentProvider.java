...
public class ContactContentProvider extends ContentProvider {
    ...
    static
    {
        ...
        MATCHER.addURI("com.tricheer.test.phone.contact", "data", CODE_NOPARAM);

    }

    @Override
    public boolean onCreate() {
        dbHelper = new DatabaseHelper(this.getContext(), "contact.db", null, 1);
        return true;
    }
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        if(db.isOpen()){
            // 特别说一下第二个参数是当name字段为空时，将自动插入一个NULL。
            long rowid = db.insert("data", "null", contentValues);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            int count=db.delete("data",selection, selectionArgs);
            this.getContext().getContentResolver().notifyChange(uri, null);
            db.close();
            return count;
        }
    ...
    public int update(Uri uri, ContentValues contentValues, String selection, String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            int count=db.update("data", contentValues, selection, selectionArgs);
            this.getContext().getContentResolver().notifyChange(uri,null);
            db.close();
            return count;
        }
